...
extern u8 T100ms, T1s;
volatile unsigned int Timer2_cnt=0;
volatile unsigned int Timer3_cnt=0;


void TIM2_IRQHandler(void)   // 1ms 주기로 호출
{
     TIM_ClearITPendingBit(TIM2, TIM_IT_Update); // Clear the interrupt flag

     // 1msec마다 카운트, 1초(1000msec)이 되면 0으로 리셋
     Timer2_cnt++;                         
     if(Timer2_cnt >999){                 // 0.1초가 되면 리셋
          Timer2_cnt = 0 ;
     }
}
...
void TIM3_IRQHandler(void)              // 1ms 주기로 호출
{
